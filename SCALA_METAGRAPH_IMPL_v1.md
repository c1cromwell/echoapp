# Scala Metagraph L1 Implementation Spec (v1.0)
...
│               ├── GovernanceVoteValidator.scala   # One-vote-per-DID, stake check
...
object TokenGenesis {
  ...
  def buildGenesisAllocations(config: GenesisConfig): Seq[GenesisAllocation] = {
    ...
    val treasuryAllocations = Seq(
      GenesisAllocation(config.treasuryMultisigAddress, 250_000_000_00000000L, Treasury)
    )
...
object GovernanceVoteValidator {
  ...
  def validate(vote: GovernanceVote, state: LedgerState): ValidationResult = {
    for {
      ...
      _ <- validateMinimumStake(vote.did, state)
      _ <- validateTrustTier(vote.did, state) // Tier 3+ for governance
  ...
  private def validateMinimumStake(did: String, state: LedgerState) = {
    val staked = state.stakedBalance(did)
    if (staked < GovernanceConfig.MinStakeToVote)
      Left(s"Insufficient stake for governance: need ${GovernanceConfig.MinStakeToVote}, have $staked")
    else Right(())
  }
}
```
...
| P1 | GovernanceVote validator (Data L1) | 3 days | Phase 1 |
