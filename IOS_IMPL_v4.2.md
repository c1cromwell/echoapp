...
**Changes from v4.1:** Updated tokenomics references (18% founders, 22% treasury). Added governance UI (proposal list, voting flow, weight display). Added Midnight evaluation note for Phase 3+. Updated founder vesting amounts (CEO 100M, co-founders 20M).
...
│   │
│   ├── Governance/                       # Trust-tier weighted governance (Phase 4)
│   │   ├── GovernanceCoordinator.swift
│   │   ├── ProposalListView.swift        # Active proposals with tally progress
│   │   ├── ProposalListViewModel.swift
│   │   ├── ProposalDetailView.swift      # Full proposal + voting interface
│   │   ├── VoteConfirmationView.swift    # Review weight + submit vote
│   │   ├── GovernanceWeightView.swift    # User's voting power breakdown
│   │   ├── GovernanceService.swift       # API calls: proposals, votes, power
│   │   └── GovernanceModels.swift        # Proposal, Vote, VotingPower, ProposalTally
│   │
│   ├── Trust/
│   │   ├── TrustTierView.swift
...
## 7. Governance Voting UI (Phase 4)

### Voting Power Display

Shown in the Wallet tab and Governance section. Displays the user's trust-tier weighted governance power.
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                LabeledContent("Staked ECHO") {
                    Text(power.totalStaked.formatted())
                ...
                LabeledContent("Formula") {
                    Text("\(power.totalStaked.formatted()) × \(power.multiplier, specifier: "%.1f")")
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if let tally = proposal.tally {
                VStack(spacing: 6) {
                    GeometryReader { geo in
                        HStack(spacing: 2) {
                            Rectangle()
                                ...
                                .frame(width: geo.size.width * tally.forPercent / 100)
                            
                            Rectangle()
                                .fill(Color.Echo.error.opacity(0.6))
                                .frame(width: geo.size.width * (100 - tally.forPercent - tally.abstainPercent) / 100)
                    ...
                    HStack {
                        Text("\(tally.forPercent, specifier: "%.0f")% For")
                            .foregroundStyle(Color.Echo.primaryContainer)
                        Spacer()
                        Text("\(tally.voterCount) voters")
                            .foregroundStyle(Color.Echo.outline)
                    }
...
struct VoteConfirmationView: View {
    let proposal: Proposal
    let voteValue: String  // "for", "against", "abstain"
    let power: VotingPower
    ...
    var body: some View {
        VStack(spacing: 24) {
            // Header
            VStack(spacing: 8) {
                Image(systemName: voteValue == "for" ? "checkmark.circle" : 
                      voteValue == "against" ? "xmark.circle" : "minus.circle")
            ...
            // Summary
            VStack(alignment: .leading, spacing: 12) {
                LabeledContent("Proposal", value: proposal.title)
                LabeledContent("Your Vote", value: voteValue.capitalized)
                LabeledContent("Your Weight", value: power.weight.formatted())
                LabeledContent("Trust Tier", value: "Tier \(power.trustTier) (\(power.multiplier, specifier: "%.1f")×)")
            }
            .font(.custom("Inter", size: 14))
            .padding(20)
            .background(
    ...
    private var voteColor: Color {
        ...
        default: return Color.Echo.outline
        }
    }
}
```
...
## 8. Design System: The Glacial Interface
...
*iOS Implementation Spec v4.2*
*Aligned to: PRD v2.5, Data Layer v3.4, Tokenomics v2.0, Hidden Folders v1.0, DESIGN.md v1.0*
