...
| 13 | Stargazer wallet vs. custom in-app token management: should ECHO build token management (balance, staking, delegation, swaps) into the iOS app, or direct users to Stargazer for all token operations? | Development scope, UX simplicity, Constellation ecosystem alignment | Phase 2 |
...
*Status: Constellation ecosystem fully integrated (v3 primitives, Digital Evidence, DeFi/liquidity). Messaging transport resolved. Deployment model resolved. Ready for Phase 1 implementation.*
