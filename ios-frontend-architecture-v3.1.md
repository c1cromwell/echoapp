# iOS Frontend Architecture (v3.1)
...
**Open question (see Data Layer v3.2, #13):** Whether ECHO builds token management (balance display, staking via TokenLock, delegation via StakeDelegation, PacaSwap swaps) into the iOS app or directs users to Stargazer wallet for all token operations.

**If in-app:** The iOS app will need a `TokenManager` that constructs Tessellation v3 transaction types (TokenLock, StakeDelegation, WithdrawLock) and submits them via the Go backend's Metagraph Gateway. Staking UI shows available validators, delegation positions, APR by tier, and withdrawal cooldown.

**If Stargazer:** The iOS app deep-links to Stargazer for all token operations. ECHO app shows read-only balance and staking status via backend cache. Reduces iOS development scope but adds UX friction (switching between apps).

Recommended: Start with Stargazer integration (Phase 2) and evaluate in-app token management for Phase 4 based on user feedback.
