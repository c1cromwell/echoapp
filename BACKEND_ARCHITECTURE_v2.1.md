# Backend Architecture & Implementation (v2.1)

## Changelog

| Version | Date | Changes |
|---------|------|---------|
| 2.1 | March 7, 2026 | Aligned with Constellation ecosystem (Tessellation v3 + Digital Evidence + DeFi). Updated Metagraph Gateway with v3 transaction types (TokenLock, StakeDelegation, AtomicAction, FeeTransaction). Added Digital Evidence API client for enterprise audit fingerprinting. Updated Anchoring Batcher with Digital Evidence relationship. Added FeeTransaction automation for snapshot fees. Updated reward pipeline to use AtomicAction. Added PacaSwap integration notes for treasury operations. |
...
| **Rewards Service** | 8004 | Reward validation, batching, AtomicAction submission | Metagraph Currency L1, Redis |
| **Contacts Service** | 8005 | Contact list, block list, search | PostgreSQL, Redis |
| **Metagraph Gateway** | 8006 | L1/L0 submission (v3 types), snapshot listening, anchoring, FeeTransaction | Metagraph nodes |
...
| **Digital Evidence** | 8010 | Enterprise audit fingerprinting, media verification, Smart Checkmark (Phase 3+) | Constellation Digital Evidence API |
...
Handles all interactions with the Constellation metagraph (Data L1 + Currency L1). All token operations use native Tessellation v3 transaction primitives for Hypergraph interoperability.
...
type MetagraphClient struct {
    ...
    deClient           *DigitalEvidenceClient // Digital Evidence API client (Phase 5)
}
...
// SubmitCurrencyL1 submits token transactions using Tessellation v3 primitives.
...
// SubmitAtomicAction bundles multiple v3 transactions into an all-or-nothing execution.
// Used for: reward claims (verify tier + claim + update cap), staking tier changes, governance vote + stake verification.
func (c *MetagraphClient) SubmitAtomicAction(actions []CurrencyL1Transaction) (string, error) {
    atomic := AtomicActionBundle{
        Actions:       actions,
        SchemaVersion: CurrentSchemaVersion,
    }
    return c.SubmitCurrencyL1(CurrencyL1Transaction{
        Type:         "atomic_action",
        AtomicBundle: &atomic,
    })
}

// PaySnapshotFees submits a FeeTransaction to pay metagraph snapshot fees from treasury DAG reserves.
// Called by AI Treasury CFO Agent on a schedule or when fees are due.
func (c *MetagraphClient) PaySnapshotFees(dagAmount int64) (string, error) {
    return c.SubmitCurrencyL1(CurrencyL1Transaction{
        Type:           "fee_transaction",
        FeeTransaction: &FeeTransactionData{Amount: dagAmount},
    })
}
...
// CurrencyL1Transaction uses Tessellation v3 transaction primitives.
type CurrencyL1Transaction struct {
    Type           string              `json:"type"`
    // v3 primitive types:
    TokenLock      *TokenLockData      `json:"tokenLock,omitempty"`      // Lock ECHO for staking
    StakeDelegation *StakeDelegationData `json:"stakeDelegation,omitempty"` // Delegate to validator
    WithdrawLock   *WithdrawLockData   `json:"withdrawLock,omitempty"`   // Initiate unstaking (14-day cooldown)
    AllowSpend     *AllowSpendData     `json:"allowSpend,omitempty"`     // Time-limited payment approval (Phase 5)
    SpendTransaction *SpendTxData      `json:"spendTransaction,omitempty"` // Execute approved spend
    FeeTransaction *FeeTransactionData `json:"feeTransaction,omitempty"` // Automated snapshot fee payment
    AtomicBundle   *AtomicActionBundle `json:"atomicBundle,omitempty"`   // Bundle multiple actions
    // Legacy types (still supported):
    Claims         []RewardClaim       `json:"claims,omitempty"`
    Transfer       *TokenTransfer      `json:"transfer,omitempty"`
}

// Tessellation v3 type definitions
type TokenLockData struct {
    DID       string `json:"did"`
    Amount    int64  `json:"amount"`
    Tier      int    `json:"tier"`       // ECHO staking tier (1-4)
    Duration  int    `json:"duration"`   // Lock duration in days
}

type StakeDelegationData struct {
    DelegatorDID string `json:"delegatorDid"`
    ValidatorID  string `json:"validatorId"`
    Amount       int64  `json:"amount"` // Amount of locked ECHO to delegate
}

type WithdrawLockData struct {
    DID    string `json:"did"`
    Amount int64  `json:"amount"`
    // 14-day cooldown enforced by Currency L1 Scala validation
}
...
type AllowSpendData struct {
    ...
    Amount     int64     `json:"amount"`
    ExpiresAt  time.Time `json:"expiresAt"` // Time-limited; never unlimited
}

type SpendTxData struct {
    AllowanceID string `json:"allowanceId"`
    Amount      int64  `json:"amount"`
}

type FeeTransactionData struct {
    Amount int64 `json:"amount"` // DAG amount for snapshot fees
}

type AtomicActionBundle struct {
    Actions       []CurrencyL1Transaction `json:"actions"`
    SchemaVersion int                     `json:"schemaVersion"`
}
...
### Digital Evidence Client (Phase 3+)

Integrates with Constellation's managed Digital Evidence REST API for enterprise audit fingerprinting, media verification, and Smart Checkmark certification. This is complementary to ECHO's custom Merkle anchoring pipeline — Merkle handles high-volume batch integrity; Digital Evidence handles individual-event enterprise compliance.
...
// DigitalEvidenceClient interacts with Constellation's Digital Evidence API.
type DigitalEvidenceClient struct {
    baseURL        string // Digital Evidence API endpoint
    apiKey         string // From Constellation subscription
    organizationID string // ECHO's organization ID
    tenantID       string // ECHO's tenant ID
    httpClient     *http.Client
}

// SubmitFingerprint anchors a SHA-256 hash on-chain via Digital Evidence.
// Returns an Event ID and public verification URL.
func (c *DigitalEvidenceClient) SubmitFingerprint(fp Fingerprint) (*FingerprintResult, error) {
    signed := c.signPayload(fp) // Constellation Metagraph Standard Signature Protocol
...
// VerifyFingerprint checks if a fingerprint exists and is valid on-chain.
func (c *DigitalEvidenceClient) VerifyFingerprint(eventID string) (*VerificationResult, error) {
    resp, err := c.get("/evidence/" + eventID)
    if err != nil {
        return nil, err
    }
...
type Fingerprint struct {
    ...
    Metadata    map[string]string `json:"metadata"`    // Event type, source, etc.
    SourceDID   string            `json:"sourceDid"`   // Originator DID
}

type FingerprintResult struct {
    EventID         string    `json:"eventId"`
    VerificationURL string    `json:"verificationUrl"` // Public Explorer URL
    Timestamp       time.Time `json:"timestamp"`
}
```
...
| Trigger | Data Fingerprinted | Phase |
|---------|-------------------|-------|
| IPFS log batch push complete | IPFS CID + batch metadata hash | Phase 5 (Org tier) |
| User sends media (optional, VIP+) | SHA-256 of image/video before E2E encryption | Phase 3 |
| Org-tier message sent | H(content + timestamp + sender DID) for Smart Checkmark | Phase 5 |
| Data retention period expires | Hash of audit batch + deletion confirmation | Phase 5 (Org tier) |
