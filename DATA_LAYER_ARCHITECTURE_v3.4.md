...
|---------|------|---------|
| 3.4 | March 26, 2026 | Revised founder allocation (18%: CEO 10%, co-founders 2% each). Treasury reduced to 22%. Added trust-tier weighted governance model to governance section. Added Midnight blockchain evaluation roadmap (Section 2.8). Single-token governance decision documented. |
...
### 2.8 Midnight Privacy Layer (Phase 3+)

Midnight is a privacy-focused partner chain to Cardano that uses zero-knowledge proofs (ZK-SNARKs) for selective disclosure. It launched mainnet in late March 2026. ECHO evaluates Midnight for privacy-credential verification after it has proven production stability.

**Decision: Cardano for identity (Phases 1-2). Add Midnight for ZK credential verification (Phase 3+).**

**What stays on Cardano (always):**
- DID Document registration (public by design — contains public keys and service endpoints)
...
|----------|----------|---------|-------|
| Trust tier verification | "Prove I am Tier 3+ without revealing my score or credential" | Eliminates hash-commitment workaround H(score\|\|nonce); native ZK verification | Phase 3-4 |
...
**Governance Model: Single Token (ECHO) + Trust-Tier Weighted Voting**

ECHO uses one token for both utility and governance. Governance power is weighted by staked ECHO multiplied by trust tier, preventing plutocratic capture without the complexity of a dual-token model.

**Governance Weight Formula (enforced by Data L1 Scala validation):**

```
GovernanceWeight = StakedECHO × TrustTierMultiplier
...
**Requirements to vote:** Staked ECHO via TokenLock (cannot vote with unstaked tokens), Trust Tier 2+, one vote per DID per proposal. Founder vesting TokenLocks are eligible for governance voting.

| Aspect | Mechanism | Timeline |
|--------|-----------|----------|
| All governance votes | Trust-tier weighted: GovernanceWeight = StakedECHO × TrustTierMultiplier. Enforced by Data L1 Scala validation. | Phase 4 (governance DAO live) |
| Data L1 schema changes | Governance proposal → weighted vote → 67% supermajority → activation at future snapshot height | Phase 4 |
...
| Validator slashing thresholds | Governance proposal → weighted vote → activation. Initial thresholds set in Section 2.1; adjustable by supermajority vote. Slashed ECHO sent to treasury. | Phase 4 |
...
| ECHO staking parameters | Staking tiers, APR rates, lock durations, minimum stakes — all governance-adjustable via weighted vote. Enforced via TokenLock validation in Currency L1 Scala code. | Phase 4 |
| Annual treasury allocation ratios | Weighted vote → simple majority → AI agents execute within approved ratios | Phase 5 |
| Cardano contract upgrades | Parameterized Plutus scripts with upgrade key (multi-sig); transition to DAO-controlled | Phase 4 |
| Midnight integration decisions | Governance vote on when to activate Midnight ZK verification features | Phase 3-4 |
...
| Metagraph token issuance changes | Require governance proposal + supermajority weighted vote; emission schedule enforced in Currency L1 Scala validation code | Phase 4 |
...
| 6 | ~~ZK proof system: Groth16 (snarkjs) or alternative (Halo2, PLONK)?~~ **RESOLVED v3.4: Use Midnight's ZK-SNARKs via Compact language (TypeScript-based). No custom ZK circuit development needed — Midnight abstracts ZK complexity. Phase 3+.** | — | — |
| 7 | ~~DAO governance: on metagraph or Cardano?~~ **RESOLVED v3.4: On metagraph (Data L1). Single ECHO token with trust-tier weighted voting. GovernanceWeight = StakedECHO × TrustTierMultiplier. Enforced by Data L1 Scala validation.** | — | — |
...
*Status: Fully aligned. Tokenomics finalized (18% founders, 22% treasury). Single-token governance with trust-tier weighting. Midnight privacy layer roadmap (Phase 3+). Ready for Phase 1 implementation.*
