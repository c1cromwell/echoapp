# Go Backend Implementation Spec (v4.0)
...
**Changes from v3.0:** Added governance service (trust-tier weighted voting: weight calculation, vote submission, proposal lifecycle, tally queries). Added governance database migrations. Updated genesis.yaml to 18% founders (CEO 100M, co-founders 20M each), 22% treasury. Renumbered sections.
...
│   │       ├── governance_handler.go  # Proposals, voting, weight calculation
...
│   │   └── models.go
│   │
│   ├── governance/                    # Governance (trust-tier weighted voting)
│   │   ├── service.go                 # Proposal CRUD, vote submission, tally query
│   │   ├── weight.go                  # GovernanceWeight = StakedECHO × TierMultiplier
│   │   ├── proposals.go               # Proposal lifecycle (create, vote, finalize)
│   │   └── models.go
│   │
...
## 6. Governance Service (Trust-Tier Weighted Voting)

```go
// governance/weight.go

package governance

// GovernanceWeight calculates trust-tier weighted voting power.
// Formula: Weight = StakedECHO × TrustTierMultiplier
// Tier 1 (Unverified): 0.0x — zero governance power
// Tier 2 (Newcomer):   0.5x
// Tier 3 (Member):     1.0x
// Tier 4 (Verified):   1.5x
// Tier 5 (Trusted):    2.0x

// TierMultipliers in basis points (10000 = 1.0x) for integer precision
...
// CalculateWeight computes governance weight for a DID.
// Includes all TokenLock positions (user staking + founder vesting).
func CalculateWeight(totalStaked int64, trustTier int) int64 {
    multiplier, exists := TierMultipliers[trustTier]
...
import (
    ...
    "time"
    "echo-backend/internal/metagraph"
    "echo-backend/internal/trust"
    "echo-backend/internal/wallet"
)
...
// GetVotingPower returns the governance weight for a DID (pre-validation).
// The authoritative calculation happens in Data L1 Scala validation.
func (s *GovernanceService) GetVotingPower(ctx context.Context, did string) (*VotingPower, error) {
    // 1. Get trust tier
    tier, err := s.trust.GetTier(ctx, did)
    if err != nil {
        return nil, err
    }

    // 2. Get total staked (all TokenLock positions including founder vesting)
    locks, err := s.wallet.GetTokenLocks(ctx, did)
    if err != nil {
        return nil, err
    }
    var totalStaked int64
    for _, lock := range locks {
        totalStaked += lock.Amount
    }

    // 3. Calculate weight
    weight := CalculateWeight(totalStaked, tier.Level)

    return &VotingPower{
        DID:         did,
        TrustTier:   tier.Level,
        Multiplier:  float64(TierMultipliers[tier.Level]) / 10000.0,
        TotalStaked: totalStaked,
        Weight:      weight,
        CanVote:     tier.Level >= 2 && totalStaked > 0,
    }, nil
}

// SubmitVote constructs and submits a governance vote to Data L1.
func (s *GovernanceService) SubmitVote(ctx context.Context, req VoteRequest) (*VoteResult, error) {
    ...
    // Submit to Data L1 (L1 validator performs authoritative weight calculation)
    txHash, err := s.metagraph.SubmitDataL1(metagraph.DataL1Submission{
        Type:          "governance_vote",
        SchemaVersion: CurrentSchemaVersion,
        Vote: &metagraph.GovernanceVoteData{
            ProposalID: req.ProposalID,
            VoterDID:   req.DID,
            Value:      req.Value, // "for", "against", "abstain"
            StakeWeight: power.TotalStaked,
...
// GetProposalTally returns current vote tallies for a proposal.
func (s *GovernanceService) GetProposalTally(ctx context.Context, proposalID string) (*ProposalTally, error) {
    return s.metagraph.QueryProposalTally(proposalID)
}
...
func (s *GovernanceService) ListActiveProposals(ctx context.Context) ([]Proposal, error) {
    return s.metagraph.QueryActiveProposals()
}
```

```go
// governance/models.go
...
type VotingPower struct {
    ...
    Multiplier  float64 `json:"multiplier"`   // e.g., 2.0 for Tier 5
    TotalStaked int64   `json:"totalStaked"`
    Weight      int64   `json:"weight"`       // StakedECHO × Multiplier
    CanVote     bool    `json:"canVote"`       // Tier 2+ and staked > 0
}

type VoteRequest struct {
    DID        string `json:"did"`
    ProposalID string `json:"proposalId"`
    Value      string `json:"value"` // "for", "against", "abstain"
}
...
type Proposal struct {
    ...
    Type        string        `json:"type"`      // "protocol_upgrade", "treasury_allocation", "parameter_change", "board_election"
    Threshold   string        `json:"threshold"` // "simple_majority", "supermajority_67", "supermajority_75"
    CreatedBy   string        `json:"createdBy"` // DID of proposer
    CreatedAt   time.Time     `json:"createdAt"`
    EndsAt      time.Time     `json:"endsAt"`
    Status      string        `json:"status"`    // "active", "passed", "failed", "executed"
...
CREATE TABLE proposals (
    ...
    -- Cached tally (updated on each vote)
...
CREATE TABLE votes (
    ...
    value       TEXT NOT NULL,  -- for, against, abstain
...
## 7. Fee Manager (FeeTransaction Automation)
  ...
  allocations:
    ...
    treasury:
      ...
      initial_distributions:
        pacaswap_liquidity: 80_000_000
        operational_reserve: 50_000_000
        locked_reserve: 90_000_000
    ...
    founders:
      ...
      allocations:
        - role: "CEO / Visionary / Product"
          share_percent: 55.56  # 100M of 180M
          amount: 100_000_000
          did: "" # Set at genesis
        - role: "CTO / Lead iOS Engineer"
          share_percent: 11.11  # 20M of 180M
          amount: 20_000_000
          did: "" # Set at genesis
